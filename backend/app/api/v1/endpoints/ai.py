...
@router.post('/initial')
def generate_initial(payload: InitialRequest) -> dict[str, object]:
    return {'root': ai_generation_service.build_initial(payload.topic)}


@router.post('/expand')
def expand_node(payload: ExpandRequest) -> dict[str, list[dict[str, str]]]:
    return {'children': ai_generation_service.expand(payload.node_text, count=payload.count)}


@router.post('/rewrite')
def rewrite_node(payload: RewriteRequest) -> dict[str, str]:
